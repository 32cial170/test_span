...
#define HSE_SRV_RSP_READ_FAILURE                ((hseSrvResponse_t)0xAA55A427UL)  /**< @brief The service request failed because read access was denied */
...
#define HSE_SRV_RSP_GENERAL_ERROR               ((hseSrvResponse_t)0x33D6D4F1UL)  /**< @brief This error code is returned if an error not covered by the error codes above is detected inside HSE. For HSE-B, it can be returned if flash programming and erase operaton was in progress at the time of giving the command */
...
#define HSE_ERR_TEMP_VIOL                  ((hseError_t)1UL << 3U) /**< @brief Temperature sensor violation.Temperature has exceeded the specified temperature range.*/
#endif
#ifdef HSE_B
#define HSE_ERR_FIRMWARE_UPDATE            ((hseError_t)1UL << 7U) /**< @brief HSE firmware update fatal error.
                                                                        This bit is set by BAF/HSE firmware during the firmware update
                                                                        operation when it encounters an exception. The HSE core will go in shutdown
                                                                        mode and will become non-operational. After completing its ongoing task, 
                                                                        application must issue a reset to the system, so that HSE FW and BAF 
                                                                        can be recovered. */
#endif /* HSE_B */
